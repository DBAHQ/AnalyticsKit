//
//  PartialAnalyticsProvider.swift
//  AnalyticsKit
//
//  Провайдер, который отправляет лишь часть событий (сейчас — Adjust: у него
//  живых событий единицы, а не весь каталог). Всё, что он не переопределил,
//  молча игнорируется.
//
//  Полноценные провайдеры (Firebase, AppMetrica) наследуют `AnalyticsProvider`
//  напрямую и обязаны реализовать каждый метод — компилятор не даст забыть
//  событие при добавлении нового.
//

import Foundation

public protocol PartialAnalyticsProvider: AnalyticsProvider {}

public extension PartialAnalyticsProvider {
